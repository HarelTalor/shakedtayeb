  ...
  return (
    <section className="relative h-[90vh] min-h-[700px] flex items-center justify-center overflow-hidden">
        ...
        <div className="flex flex-col items-center space-y-8 md:space-y-12 max-w-4xl mx-auto">
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300">
            ...
            <h1 className="text-3xl md:text-6xl font-serif font-bold tracking-tight leading-tight drop-shadow-2xl">
              אמנות הדיוק והיופי הטבעי
            </h1>
            
            <p className="text-lg md:text-2xl font-normal opacity-90 max-w-2xl mx-auto leading-relaxed tracking-wide">
              [טקסט דמה המתאר את החזון המקצועי של הקליניקה והמחויבות למצוינות]
            </p>
          </div>
      ...
      <div className="absolute bottom-6 left-0 right-0 z-30 flex flex-col items-center cursor-pointer group animate-bounce"
        ...
        <span className="text-white/60 text-[10px] font-serif tracking-[0.3em] uppercase mb-1 drop-shadow-md group-hover:text-white transition-colors">
          גללי למטה
        </span>
        <ChevronDown className="w-8 h-8 text-white/70 group-hover:text-white transition-colors stroke-[1.5px]" />
      </div>
    </section>
  );
}
